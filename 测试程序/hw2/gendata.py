...
intPool = [0, 1,0,0,2,3,34,5,66,4,3,9999999999999999]  # 常量池
...
def genData():
    ...
    while(len(expr)>100):
        expr=getExpr(0)
    sym = expr.replace("f", "(" + ff + ")")
    expr=expr.replace("g","g(x,y,z)").replace("h","h(x,y,z)").replace("f","f(("+f1+"),("+f2+"),("+f3+"))")
    sym = sym.replace("g", "("+g+")")
    sym = sym.replace("h", "("+h+")")
    x = sympy.Symbol('x')
    y=sympy.Symbol('y')
