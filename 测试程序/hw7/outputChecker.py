...
class Elevator:
    def __init__(self, str):
        ...
        if eles[1] == 'Elevator':
            ...
            self.access=int(eles[6])
        else:
            ColorfulPrint.colorfulPrint('??? Unknown elevator type ???', ColorfulPrint.MODE_BOLD, ColorfulPrint.COLOR_RED)
    ...
    def checkOpen(self, eles, lineNum, opTime):
        serveNum[int(eles[1])] = serveNum[int(eles[1])] + 1;
        if serveNum[int(eles[1])] > 4:
            printError('serveNum error!', lineNum)
        if self.state != STATE_CLOSE:
            printError('Elevator door is not closed before open!', lineNum)
        self.state = STATE_OPEN
        self.lastOpenTime = opTime
        if (1<<(int(eles[1])-1))&self.access==0 and self.inmaintain==False:
            printError('Elevator cannot open here!',lineNum)
    ...
    def checkClose(self, eles, lineNum, opTime):
        serveNum[int(eles[1])] = serveNum[int(eles[1])] - 1
        if self.out==False:
            inNum[int(eles[1])]=inNum[int(eles[1])]-1
        self.out=False
        self.firstIn=True
    ...
    def checkIn(self, eles, lineNum, opTime):
        if self.firstIn==True:
            if self.out==False:
                inNum[int(eles[2])]=inNum[int(eles[2])]+1
            if inNum[int(eles[2])]>2:
                printError('inNum error!',lineNum)
            self.firstIn=False
        if self.state != STATE_OPEN:
...
def initElevator():
    ...
    flag[0] = 1
    lastOpTime[0] = -1.0
    elevators[1] = Elevator('[0.0]ADD-Elevator-1-1-6-0.4-2047')
    elevators[2] = Elevator('[0.0]ADD-Elevator-2-1-6-0.4-2047')
    elevators[3] = Elevator('[0.0]ADD-Elevator-3-1-6-0.4-2047')
    elevators[4] = Elevator('[0.0]ADD-Elevator-4-1-6-0.4-2047')
    elevators[5] = Elevator('[0.0]ADD-Elevator-5-1-6-0.4-2047')
    elevators[6] = Elevator('[0.0]ADD-Elevator-6-1-6-0.4-2047')
