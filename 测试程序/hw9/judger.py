    ...
    for i in range(len(ans[0])):
        for j in range(playerNum):
            if j == 0:
                ...
                if cmp != ans[j][i]:
                    flag = False
                    ColorfulPrint.colorfulPrint('Error in line: ' + str(i + 1) + '!'+' '+str(j), ColorfulPrint.MODE_BOLD, ColorfulPrint.COLOR_RED)
                    waCount += 1
...
if __name__ == '__main__':
    #time1=time.time()
    #exec('java -jar homework9.jar < stdin.txt > stdout.txt')
    #time2=time.time()
    #print(time2-time1)
